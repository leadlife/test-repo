...
for( i in 1:(NIRorder-1))
{
 ...
 for (j in (i+1):NIRorder)
 {
  ...
  CovBeta <- CovBetatemp%*%t(CovBetatemp);
  Xmatr1 <- cbind(hermite(tempx1,0),hermite(tempx1,1),hermite(tempx1,2),hermite(tempx1,3),hermite(tempx1,4));
  Xmatr2 <- cbind(hermite(tempx2,1),hermite(tempx2,2),hermite(tempx2,3),hermite(tempx2,4));
  Xmatr <- cbind(Xmatr1,Xmatr2);
  Varyhat2 <- (1+Xmatr%*%CovBeta%*%t(Xmatr))*Merge2Result[[8]];
  yhat2[i,j] <- TangentCheck2(Xmatr,tempx1,Merge2Result[[1]]);
...
tempx <- -1;
XmatrN1 <- cbind(hermite(tempx,0),hermite(tempx,1),hermite(tempx,2),hermite(tempx,3),hermite(tempx,4));
Tangentch <- TangentCheck1(XmatrN1,tempx,Beta);
InterceptN1 <- Tangentch[[2]];
SlopeN1 <- Tangentch[[1]];
tempx <- 0;
Xmatr0 <- cbind(hermite(tempx,0),hermite(tempx,1),hermite(tempx,2),hermite(tempx,3),hermite(tempx,4));
Tangentch0 <- TangentCheck1(Xmatr0,tempx,Beta);
Intercept0 <- Tangentch0[[2]];
Slope0 <- Tangentch0[[1]];
MinErrIndex 
IR1max
yhat1	
...
ablineclip(a=InterceptN1,b=SlopeN1,x1=-2,x2=0,lwd=2);
ablineclip(a=Intercept0,b=Slope0,x1=-1,x2=1,lwd=2);
x11();
